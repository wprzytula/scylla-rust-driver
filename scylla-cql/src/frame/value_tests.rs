...
// so without it it throws a warning about being unised.
...
fn map_value_list() {
    ...
    let new_values = serialize_values(row.clone(), cols);
    let mut new_values_bytes = Vec::new();
    new_values.write_to_request(&mut new_values_bytes);
    assert_eq!(
        new_values_bytes,
        vec![
            0, 3, // value count: 3
