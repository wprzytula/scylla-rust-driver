...
... and `RawIterator` into `LegacyRowIterator`:

```rust
# extern crate scylla;
# use scylla::transport::iterator::{RawIterator, LegacyRowIterator};
# use std::error::Error;
# async fn check_only_compiles(iter: RawIterator) -> Result<(), Box<dyn Error>> {
let iter: RawIterator = iter;
let legacy_result: LegacyRowIterator = iter.into_legacy();
# Ok(())
# }
```
