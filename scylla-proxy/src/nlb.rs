...
//!
//! [`NlbFrontend`] listens on a single address and round-robins incoming
...
/// A lightweight L4 TCP load balancer that round-robins connections across backends.
///
...
impl NlbFrontend {
    ...
    pub async fn run(self) -> Result<RunningNlbFrontend, io::Error> {
        ...
        let handle = tokio::task::spawn(async move {
            let mut terminate_rx = terminate_tx_clone.subscribe();
            loop {
                tokio::select! {
                    result = listener.accept() => {
                        match result {
                            Ok((stream, peer_addr)) => {
                                ...
                                let mut conn_terminate = terminate_tx_clone.subscribe();
                                tokio::task::spawn(async move {
                                    tokio::select! {
                                        result = Self::handle_connection(stream, peer_addr, &shared) => {
                                            if let Err(e) = result {
                                                debug!("NLB connection from {} ended: {}", peer_addr, e);
                                            }
                                        }
                                        _ = conn_terminate.recv() => {
                                            debug!("NLB connection from {} terminated by shutdown", peer_addr);
                                        }
                                    }
                                });
                            }
                    ...
                    _ = terminate_rx.recv() => {
                        info!("NLB frontend shutting down");
                        break;
                    }
                }
            }
    ...
    async fn handle_connection(
        ...
        // Pick the next backend in round-robin fashion.
        ...
        let backend_addr = {
            ...
            let idx = shared.counter.fetch_add(1, Ordering::Relaxed) % backends.len();
            backends[idx]
        };
...
mod tests {
    ...
    /// Smoke test: NLB round-robins connections across two backends.
    #[tokio::test]
    async fn test_nlb_round_robin() {
        ...
        // Make two connections. They should go to different backends.
        let mut conn1 = TcpStream::connect(nlb_addr).await.unwrap();
        conn1.write_all(b"hi").await.unwrap();
        let mut buf1 = [0u8; 3];
        conn1.read_exact(&mut buf1).await.unwrap();

        let mut conn2 = TcpStream::connect(nlb_addr).await.unwrap();
        conn2.write_all(b"hi").await.unwrap();
        let mut buf2 = [0u8; 3];
        conn2.read_exact(&mut buf2).await.unwrap();

        // The two connections should have hit different backends (round-robin).
        assert_ne!(
            buf1[0], buf2[0],
            "Expected round-robin to different backends, but both went to the same one"
        );
        assert_eq!(&buf1[1..], b"hi");
        assert_eq!(&buf2[1..], b"hi");
