...
impl GenericSession<LegacyDeserializationApi> {
    ...
    pub async fn query_paged(
        ...
        Ok(self
            .do_query_paged(query.into(), values, paging_state)
            .await?
            .into_legacy_result()?)
    }
    ...
    pub async fn execute_paged(
        ...
        Ok(self
            .do_execute_paged(prepared, values, paging_state)
            .await?
            .into_legacy_result()?)
    }
    ...
    async fn try_getting_tracing_info(
        ...
        let maybe_tracing_info: Option<TracingInfo> = traces_session_res
            ...
            .map_err(|err| match err {
                ...
                RowsError::DeserializationFailed(_) => QueryError::ProtocolError(
        ...
        let tracing_event_rows = traces_events_res.rows().map_err(|err| match err {
            ...
            RowsError::DeserializationFailed(_) => {
                QueryError::ProtocolError("Failed to deserialize columns from system_traces.events")
            }
        })?;

        tracing_info.events = tracing_event_rows
            .collect::<Result<_, _>>()
            .map_err(|err| QueryError::InvalidMessage(err.to_string()))?;
...
impl RequestSpan {
    ...
    pub(crate) fn record_result_fields(&self, result: &QueryResult) {
        self.span.record("result_size", result.rows_size());
        self.span.record("result_rows", result.rows_num());
    }
