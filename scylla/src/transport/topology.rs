...
async fn query_peers(conn: &Arc<Connection>, connect_port: u16) -> Result<Vec<Peer>, QueryError> {
    ...
    let peers_query_stream = conn
        ...
        .map(|it| {
            it.and_then(|it| {
                it.into_typed::<NodeInfoRow>()
                    .map_err(Into::into)
                    .map(|iter| iter.into_stream())
            })
        })
        .into_stream()
        .try_flatten()
...
where
    R: for<'r> DeserializeRow<'r> + 'static,
{
    let conn = conn.clone();
    ...
    let fut = async move {
        ...
        .and_then(|it: super::iterator::RawIterator| it.into_typed::<R>().map_err(Into::into))
        .map(|row_iterator| row_iterator.into_stream())
    };
...
async fn query_table_partitioners(
    ...
    let rows = conn
        ...
        .map(|it| {
            it.and_then(|it| {
                it.into_typed::<(String, String, Option<String>)>()
                    .map_err(Into::into)
                    .map(|iter| iter.into_stream())
            })
        })
