...
impl QueryResult {
    ...
    ///
    /// ```
    pub fn rows_deserializer(&self) -> Result<Option<RowsDeserializer<'_>>, RowsParseError> {
        self.raw_rows
            .as_ref()
            .map(|raw_rows| {
    ...
    /// ) -> Result<RowsDeserializer<'static>, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_query_result() {
        ...
        fn sample_raw_rows(cols: usize, rows: usize) -> RawRows {
            ...
            RawRows::new_for_test(None, Some(metadata), false, rows, &bytes).unwrap()
        }
        ...
        fn sample_raw_rows_invalid_bytes(cols: usize, rows: usize) -> RawRows {
            ...
            RawRows::new_for_test(None, Some(metadata), false, rows, &[]).unwrap()
        }
        ...
        // Check col specs
        {
            ...
            // RESULT::Rows response -> some column specs
            {
                ...
                let rr = RawRows::new_for_test(None, Some(metadata), false, 0, &[]).unwrap();
