...
use scylla_cql::frame::response::result::{
    ColumnSpec, ColumnType, RawRows, RawRowsWithDeserializedMetadata, Row, TableSpec,
};
...
/// To create a [`RowsDeserializer`], use [`QueryResult::row_deserializer`] method.
...
impl QueryResult {
    ...
    ///
    /// TODO: code example
    pub fn rows_deserializer<'frame>(
        &'frame self,
    ) -> Result<Option<RowsDeserializer<'frame>>, RowsParseError> {
        self.raw_rows
            .as_ref()
            .map(|raw_rows| {
    ...
    pub fn rows_deserializer_owned(
        &self,
    ) -> Result<Option<RowsDeserializer<'static>>, RowsParseError> {
        self.raw_rows
            .as_ref()
            .map(|raw_rows| {
    ...
    pub fn into_legacy_result(self) -> Result<LegacyQueryResult, RowsParseError> {
        if let Some(raw_rows) = self.raw_rows {
            ...
            let serialized_size = raw_rows_with_metadata.rows_size();
...
/// TODO: code example
#[derive(Debug)]
pub struct RowsDeserializer<'frame> {
    raw_rows_with_metadata: RawRowsWithDeserializedMetadata<'frame>,
}

impl<'frame> RowsDeserializer<'frame> {
    ...
    pub fn rows_size(&self) -> usize {
        self.raw_rows_with_metadata.rows_size()
    }
...
mod tests {
    ...
    fn test_query_result() {
        ...
        fn sample_raw_rows(cols: usize, rows: usize) -> RawRows {
            ...
            RawRows::new_for_test(None, Some(metadata), false, rows, bytes)
        }
        ...
        fn sample_raw_rows_invalid_bytes(cols: usize, rows: usize) -> RawRows {
            ...
            RawRows::new_for_test(None, Some(metadata), false, rows, Bytes::new())
        }
        ...
        // Check col specs
        {
            ...
            // RESULT::Rows response -> some column specs
            {
                ...
                let rr = RawRows::new_for_test(None, Some(metadata), false, 0, Bytes::new());
