...
impl QueryResult {
    ...
    pub fn maybe_rows<'s, R: DeserializeRow<'s>>(
        &'s self,
    ) -> Result<Option<TypedRowIterator<'s, R>>, TypeCheckError> {
        self.raw_rows
            .as_ref()
    ...
    pub fn maybe_first_row<'s, R: DeserializeRow<'s>>(&'s self) -> Result<Option<R>, RowsError> {
        self.rows::<R>()?
            .next()
            .transpose()
            .map_err(RowsError::DeserializationFailed)
    }
    ...
    pub fn first_row<'s, R: DeserializeRow<'s>>(&'s self) -> Result<R, FirstRowError> {
        match self.maybe_first_row::<R>() {
            ...
            Err(RowsError::DeserializationFailed(err)) => {
    ...
    pub fn single_row<'s, R: DeserializeRow<'s>>(&'s self) -> Result<R, SingleRowError> {
        match self.rows::<R>() {
            ...
            Err(RowsError::DeserializationFailed(err)) => {
                Err(SingleRowError::DeserializationFailed(err))
            }
        }
    }

    // TODO: think how to make this API more forward-compatible,
    // by exposing a new ColumnSpecView and hiding implementation details,
    // because those are inner types that may need to be changed in the future.
    //
