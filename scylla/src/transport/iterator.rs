...
///
/// A pre-0.14.0 interface is also available, although deprecated:
...
/// a `next()` method that returns a ColumnIterator<'r>, which can be used
...
impl RawIterator {
    ...
    /// Can be used with `next()` for manual deserialization. See `next()` for example.
    #[inline]
    pub fn type_check<'frame, RowT: DeserializeRow<'frame>>(&self) -> Result<(), TypeCheckError> {
    ...
    #[inline]
    pub fn into_typed<'frame, RowT: DeserializeRow<'frame>>(
        self,
...
impl<'frame, RowT> TypedRowIterator<RowT>
where
    RowT: DeserializeRow<'frame>,
{
    ...
    pub async fn next(&'frame mut self) -> Option<Result<RowT, QueryError>> {
        self.raw_iterator.next().await.map(|res| {
            res.and_then(|column_iterator| {
                <RowT as DeserializeRow>::deserialize(column_iterator).map_err(Into::into)
            })
        })
    }
}
    ...
    RowT: for<'r> DeserializeRow<'r>,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let next_fut = s.typed_row_iterator.raw_iterator.next();
        futures::pin_mut!(next_fut);
        let iter = ready_some_ok!(next_fut.poll(cx));
        let value = <RowT as DeserializeRow<'_>>::deserialize(iter)
...
impl Stream for LegacyRowIterator {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let next_ready: Option<Self::Item> = match next_elem {
            Some(Ok(iter)) => Some(Row::deserialize(iter).map_err(|e| ParseError::from(e).into())),
