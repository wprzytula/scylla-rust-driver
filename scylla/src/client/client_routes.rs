...
mod tests {
    ...
    // translation. It should gracefully return the original untranslated address,
    // because the driver needs to be able to connect to nodes before the first
    // metadata refresh populates the client_routes table contents.
    #[tokio::test]
    async fn translate_address_returns_original_when_no_client_routes() {
        ...
        let peer = make_peer(Uuid::new_v4(), original_addr);

        let result = translator.translate_address(&peer).await.unwrap();
        assert_eq!(result, original_addr);
    }

    // In a mixed cluster, some nodes may be reachable via client_routes and some may
    // not. Nodes that are not part of the custom routing setup will not appear in the
    // client_routes table, so their host_id won't be found in the routes map.
    // In that case, the translator should return the original address so the driver
    // can still connect to those nodes directly.
    #[tokio::test]
    async fn translate_address_returns_original_when_host_id_not_in_routes() {
        let config = make_config(vec![ClientRoutesProxy::new_with_connection_id(
            "conn-1".to_string(),
        )]);
        let translator = ClientRoutesAddressTranslator::new(config, None, false);
    ...
    // returns the original address (no translation possible); after
    // replace_client_routes is called, the translator uses the stored routes
    // for subsequent translate_address calls.
    #[tokio::test]
    async fn update_client_routes_makes_subsequent_translations_use_routes() {
        ...
        // Before updating: no routes stored, so original address is returned.
        let result = translator.translate_address(&peer).await.unwrap();
        assert_eq!(result, original_addr);
    ...
    async fn replace_client_routes_removes_hosts_absent_from_snapshot() {
        ...
        // host_b was removed →  falls back to untranslated address.
        assert_eq!(
            translator.translate_address(&peer_b).await.unwrap(),
            localhost_addr(19998)
        );
    }
    ...
    async fn merge_client_routes_update_removes_dangling_entries() {
        ...
        // Dangling (absent): host_b removed →  falls back to untranslated.
        assert_eq!(
            translator.translate_address(&peer_b).await.unwrap(),
            localhost_addr(19998)
        );
