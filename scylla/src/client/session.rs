...
impl Session {
    ...
    async fn prepare_nongeneric(
        ...
        async fn preparation_worker(
            ...
            // Whether the first successfully prepared statement has already been sent through the channel.
            let sent_prepared = AtomicBool::new(false);
            let sent_prepared_ref = &sent_prepared;
            ...
            let preparations = connections_iter.map(|c| async move {
                ...
                if res.is_err() || !sent_prepared_ref.load(std::sync::atomic::Ordering::Relaxed) {
                    let _ = tx_ref
                        .send(res.map(|raw| raw.into_prepared_statement()))
                        .await;
                }
