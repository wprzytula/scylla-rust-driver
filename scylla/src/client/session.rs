...
impl Session {
    ...
    pub async fn execute_unpaged(
        ...
        self.execute_unpaged_nongeneric(prepared, &serialized_values)
            .await
    }

    async fn execute_unpaged_nongeneric(
        &self,
        prepared: &PreparedStatement,
    ...
    ///
    /// Requires a [BoundStatement] generated by binding values to a [PreparedStatement].
    ///
    /// Performs an unpaged request, i.e. all results are received in a single response.
    ///
    /// As all results come in one response (no paging is done!), the memory footprint and latency may be huge
    /// for statements returning rows (i.e. SELECTs)! Prefer this method for non-SELECTs, and for SELECTs
    /// it is best to use paged requests:
    /// - to receive multiple pages and transparently iterate through them, use [execute_bound_iter](Session::execute_bound_iter).
    /// - to manually receive multiple pages and iterate through them, use [execute_bound_single_page](Session::execute_bound_single_page).
    ///
    /// > ***Warning***
    /// >
    /// > For token/shard aware load balancing to work properly, all partition key values
    ...
    /// > (see [performance section](https://rust-driver.docs.scylladb.com/stable/statements/prepared.html#performance)).
    ///
    /// See [the book](https://rust-driver.docs.scylladb.com/stable/statements/bound.html) for more information.
    ///
    /// # Arguments
    /// * `bound` - the bound statement to execute, generated by binding values to a [PreparedStatement]
    ///
    /// # Example
    /// ```rust
    /// # use scylla::client::session::Session;
    ...
    /// # }
    /// ```
    pub async fn execute_bound_unpaged(
        &self,
        bound: &BoundStatement,
    ) -> Result<QueryResult, ExecutionError> {
        self.execute_unpaged_nongeneric(&bound.prepared, &bound.values)
            .await
    }
    ...
    pub async fn execute_single_page(
        ...
        let serialized_values = prepared.serialize_values(&values)?;
        self.execute_single_page_nongeneric(prepared, &serialized_values, paging_state)
            .await
    }

    async fn execute_single_page_nongeneric(
        &self,
        prepared: &PreparedStatement,
        serialized_values: &SerializedValues,
        paging_state: PagingState,
    ...
    /// Executes a bound statement, restricting results to single page.
    ///
    /// Optionally continues fetching results from a saved point.
    ///
    /// Requires a [BoundStatement] generated by binding values to a [PreparedStatement].
    ...
    ///     )
    ///     .await?;
    /// let paged_bound = paged_prepared.bind(&())?;
    ///
    /// // Manual paging in a loop, prepared statement.
    /// let mut paging_state = PagingState::start();
    /// loop {
    ///     let (res, paging_state_response) = session
    ///         .execute_bound_single_page(&paged_bound, paging_state)
    ///         .await?;
    ///
    ///    // Do something with a single page of results.
    ///    for row in res
    ...
    /// # }
    /// ```
    pub async fn execute_bound_single_page(
        &self,
        bound: &BoundStatement,
        paging_state: PagingState,
    ) -> Result<(QueryResult, PagingStateResponse), ExecutionError> {
        self.execute_single_page_nongeneric(&bound.prepared, &bound.values, paging_state)
            .await
    }
    ...
    /// Execute a bound statement with paging.
    ///
    /// This method will query all pages of the result.
    ///
    /// Requires a [BoundStatement] generated by binding values to a [PreparedStatement].
    ...
    /// # }
    /// ```
    pub async fn execute_bound_iter(
        &self,
        bound: BoundStatement,
    ) -> Result<QueryPager, PagerExecutionError> {
        self.execute_iter_nongeneric(bound.prepared, bound.values)
            .await
    }
...
/* Bound statements */
impl sealed::Sealed for BoundStatement {}
impl Executable for BoundStatement {
    async fn unpaged(self, session: &Session) -> Result<QueryResult, ExecutionError> {
        let bound = self;
        session.execute_bound_unpaged(&bound).await
    }
...
impl Executable for &BoundStatement {
    ...
    async fn single_page(
        ...
        let bound = self;
        session.execute_bound_single_page(bound, paging_state).await
    }
