...
impl ClientRoutesCluster {
    ...
    /// 2. CCM cluster start
    /// 3. Discover each node's `host_id` via a direct CQL connection
    /// 4. Start per-node proxy chains
    /// 5. POST client routes via Scylla's REST API on each node
    async fn setup(opts: ClusterOptions) -> Result<Self, Error> {
        ...
        // Step 3: discover host IDs (uses plaintext port 9042).
    ...
    ///
    /// Installs a single request rule on every proxy that detects a `PREPARE`
    /// on the control connection whose body contains `"client_routes"`. After
    /// a malformed event breaks the CC, the driver reconnects and creates a
    /// fresh prepared-statement cache, so the `SELECT … FROM
    /// system.client_routes` query is re-PREPAREd.
    ///
    ...
    pub(crate) fn setup_malformed_event_requery_detection(
        ...
        for dc_cfg in self.dc_configs.values_mut() {
            for (&node_id, chain) in dc_cfg.per_node_chains.iter_mut() {
                ...
                // Detect the metadata refresh (PREPARE for
                // system.client_routes on the new control connection).
                let requery_condition = Condition::RequestOpcode(RequestOpcode::Prepare)
                    .and(Condition::ConnectionRegisteredAnyEvent)
                    .and(Condition::BodyContainsCaseSensitive(
                        b"client_routes".to_vec().into_boxed_slice(),
                    ));
    ...
    /// Mutable access to the underlying CCM cluster.
    ...
    ///
    /// Times out after `timeout` to avoid hanging forever if the driver
    /// fails to connect (e.g., due to the `Untranslatable` bug).
    ...
    ///
    /// Steps:
    /// 1. Post updated routes (without this node) to all remaining nodes
    /// 2. Decommission the node via CCM
    /// 3. Delete the node from CCM
    ...
    pub(crate) async fn decommission_node(&mut self, node_id: NodeId) -> Result<(), Error> {
        ...
        // Remove the host_id entry.
        self.host_ids.remove(&node_id);

        // Remove the chain from dc_configs.
        let chain = self
            .dc_configs
            .get_mut(&dc_id)
        ...
        // Step 1: post updated routes (without this node) BEFORE topology change.
        self.post_routes_to_all_nodes()
            .await
            .context("Failed to post updated routes before decommission")?;

        // Update the per-DC contact-point NLB to exclude the decommissioned node.
        if let Some(dc_cfg) = self.dc_configs.get(&dc_id) {
            dc_cfg.refresh_contact_point_backends();
        }

        // Step 2+3: decommission and delete from CCM.
    ...
    /// Steps:
    /// 1. CCM add the node
    /// 2. CCM start the node
    /// 3. Discover the new node's host_id
    /// 4. Start proxy chain
    /// 5. Post updated routes (with the new node) to all nodes
    ///
    ...
    /// Build the route list and POST to all reachable cluster nodes via REST API.
    ///
    /// Only posts to nodes that have an active proxy chain (i.e., are running).
    /// Stopped or decommissioned nodes are skipped because their REST API is
    /// unreachable.
    ...
    T: AsyncFnOnce(&mut ClientRoutesCluster) -> (),
{
    ...
    let mut plc = ClientRoutesCluster::setup(opts)
        .await
