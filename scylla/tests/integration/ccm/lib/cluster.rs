...
impl Cluster {
    ...
    pub(crate) async fn add_node(
        ...
        // Compute the node's IP from the cluster prefix and pass it
        // explicitly via `-i`. CCM's `add` command does not always
        // inherit the cluster's IP prefix, so being explicit is safer.
