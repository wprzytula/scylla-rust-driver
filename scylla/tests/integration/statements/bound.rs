...
use scylla::statement::bound::{
    AppendingStatementBinderError, BoundStatement, ByIndexStatementBinderError,
    ByNameStatementBinderError,
};
use scylla::statement::{Consistency, SerialConsistency, Statement};
use scylla::value::Unset;
...
async fn test_binders() {
    ...
    // `AppendingStatementBinder`
    {
        // Correct case.
        {
            let binder = prepared.clone().into_binder().appending_binder();
    ...
    // `ByIndexStatementBinder`
    {
        // Correct case.
        {
            let binder = prepared.clone().into_binder().by_index_binder(&Unset);
            let bound = binder
        ...
        // Static lifetime check: the owned binder can be used after the original prepared statement is dropped.
        {
            let _bound = {
                let prepared = prepared.clone();
                let binder = prepared.into_binder().by_index_binder(&Unset);
                binder
                    .bind_value_by_index(0, &"local")
                    .unwrap()
                    .finish()
    ...
    // `ByNameStatementBinder`
    {
        // Correct case.
        {
            let binder = prepared.clone().into_binder().by_name_binder(&Unset);
            let bound = binder
        ...
        // Static lifetime check: the owned binder can be used after the original prepared statement is dropped.
        {
            let _bound = {
                let prepared = prepared.clone();
                let binder = prepared.into_binder().by_name_binder(&Unset);
                binder
                    .bind_value_by_name("key", &"local")
                    .unwrap()
                    .finish()
